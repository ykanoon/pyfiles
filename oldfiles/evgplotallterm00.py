...
#this version plot geodetic deformation period in Takagi2019
...
#data plot
	...
	if tfer.empty:
		...
		for rnum in range(len(tfer)):
			print(tfer.ErStart[rnum])
			ax1.axvspan(tfer.ErStart[rnum],tfer.Erend[rnum], color="red",alpha=0.7)
...
#plot GNSS files
	...
	for fnum in range(len(filenames)):
		...
		ax2.plot(df-df['dll(mm)'].mean(),ls='None',markersize=1,marker=".",label=labelname,color=cm.jet(fnum/len(filenames)))
	...
	if tfer.empty:
		...
		for rnum in range(len(tfer)):
			#print(tfer.ErStart[rnum])
			ax2.axvspan(tfer.ErStart[rnum],tfer.Erend[rnum], color="red",alpha=0.7)
		

	plt.tight_layout()

	plt.savefig(dirname+'/Figs/allterm/figure'+mtname+'.png') # -----(2)
	plt.close()
	plt.clf() #------(3
