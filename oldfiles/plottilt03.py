...
trgi = dfCD[ dfCD[NS].diff().abs()>td  ].index
dfNS = dfCD['ONTA_NS_C'].diff()
dfEW = dfCD['ONTA_EW_C'].diff()

print( len(trgi) )

for tnum in range(len(trgi)):
	print(trgi[tnum])
	print(dfNS.loc[ dfNS.index == trgi[tnum] ].iloc[0])
	print( dfCD[ dfCD.index == trgi[tnum]  ].ONTA_NS_C )
	#dfCD.loc[ dfCD.index < trgi[tnum] , 'ONTA_NS_C'] = dfCD.loc[ dfCD.index < trgi[tnum] , 'ONTA_NS_C'] + dfNS.loc[ dfNS.index == trgi[tnum] ].iloc[0] 
	#dfCD.loc[ dfCD.index < trgi[tnum] , 'ONTA_EW_C'] = dfCD.loc[ dfCD.index < trgi[tnum] , 'ONTA_EW_C'] + dfEW.loc[ dfEW.index == trgi[tnum] ].iloc[0]
