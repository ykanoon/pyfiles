...
#data plot
	...
	tmp = pd.read_csv(dirname+'DLname.csv')
	
	tmp = tmp.query('mtname == "AZM"')
	tmp = tmp.reset_index()
	print('filename'+tmp.loc[1,['code1']])
	print(len(tmp))
	...
	print(glob.glob(dirname+'GNSScsv/*J831*J832*'))
	...
	for filename in filenames:
		...
		ax2.plot(df-df['dll(mm)'].mean(),ls='None',marker=".",label=filename[-17:-8])
		ax2.set_title('GNSS data')
		ax2.set_xlim([xmin,xmax])
		ax2.set_xlabel('Time')
		ax2.set_ylabel('DL (mm)')

	ax2.legend(bbox_to_anchor=(1.05,1),loc='upper left',borderaxespad=1)
	ax2.grid(which="both")
