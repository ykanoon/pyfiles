...
def cal_rho(lon_a,lat_a,lon_b,lat_b):
	...
	xx=numpy.arccos(numpy.sin(pa)*numpy.sin(pb)+\
	numpy.cos(pa)*numpy.cos(pb)*numpy.cos(rad_lon_a-rad_lon_b))
...
def importGNSS(dirname,dstart,dend,BLcodes,plotunit):
	...
	for codes in BLcodes:
		...
		if plotunit == 'strain':
			...
			rho = myfunc.cal_rho(\
			pos1[0,1],pos1[0,0],pos2[0,1],pos2[0,0])          
...
def detrenddaydf(df,dets,dete):
	...
	dtdf = df.copy()	
	tmp = dtdf[dets:dete].interpolate('linear').dropna()
		
	for column_name, item in tmp.iteritems():

		x = numpy.arange(0,len(tmp[column_name]))
		y = tmp[column_name].values
		ab = numpy.polyfit(x,y,1)

		trenddf[column_name] = ab[0]*numpy.arange(0,len(trenddf[column_name]))

		dtdf[column_name] = \
		dtdf[column_name] - \
		ab[0]*numpy.arange(0,len(dtdf[column_name]))	

		dtdf[column_name] = \
		dtdf[column_name] - dtdf[column_name].mean()


		print(tmp[column_name])	


	dtdf = dtdf.add_suffix('_det')
	trenddf = trenddf.add_suffix('_trend')
	return(dtdf, trenddf)
